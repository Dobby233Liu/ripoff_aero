...
Mod.WAVY_LAKE = love.graphics.newShader[[
    ...
    extern number clamp_chunk_dim = 1.0; // -1 = crop out, 0 = no clamping
    ...
    vec4 effect(vec4 color, Image tex, vec2 texture_coords, vec2 screen_coords) {
        ...
        if (thickness.x > 0.0) {
            number x_siner = broken_freq ? (sine + chunk.x * diff_freq) * freq : sine * freq + chunk.x * diff_freq;
            texture_coords.x += sin(x_siner) * mag / texture_dim.x;
        }
        if (thickness.y > 0.0) {
            number y_siner = broken_freq ? (sine + chunk.y * diff_freq) * freq : sine * freq + chunk.y * diff_freq;
            if (y_cos)
